...
If the master and the modules are connected through a LAN, the easiest solution is to let the master connect to a [Switch](https://github.com/gioblu/PJON/blob/master/examples/ARDUINO/Local/SoftwareBitBang/Tunneler/SwitchA/SwitchA.ino) using the LocalUDP strategy. This is efficient and requires little configuration. This version of the master is present in the WINDOWS_LINUX_LUDP directory. More switches can be added as the setup grows.
