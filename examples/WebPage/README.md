...
If the master and the modules are connected through a LAN, the easiest solution is to let the master connect to a [Switch](https://github.com/gioblu/PJON/tree/master/examples/routing/ARDUINO/Local/Tunneler/BlinkingRGBSwitch) using the DualUDP strategy. This is efficient and require little configuration. This version of the master is present in the WINDOWS_LINUX_DUDP directory. More switches can be added as the setup grows. A version of the switch has been extended to be a ModuleInterface module which reports statistics and status so that the network can be monitored. This switch is available [here](https://github.com/fredilarsen/ModuleInterface/tree/master/examples/WebPage/ARDUINO_SWBB/ModuleInterfaceRGBSwitch).
