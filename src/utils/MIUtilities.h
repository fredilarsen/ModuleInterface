...
inline bool mi_interval_elapsed(uint32_t &lastRun, uint32_t intervalMs) {
...
inline bool mi_is_after(uint32_t A, uint32_t B){ // Returns true if A later than B
	return ((A-B) & 0x80000000) == 0; // Extracting sign bit. If negative, B>A 
}

inline bool mi_after_or_equal(uint32_t A, uint32_t B){
	if (A == B) return true;
...
inline float mi_lowpass(float new_value, float previous_lowpass, float factor) {
...
inline float mi_lowpass(float new_value, float previous_lowpass, float factor, uint32_t &last_lowpass_millis, const uint32_t sample_interval) {
...
inline bool mi_compare_ignorecase(const char *a, const char *b, uint16_t len) {
  const char *pa = a, *pb = b;
  for (; pa-a < len && *pa != 0 && (*pa == *pb || tolower(*pa) == tolower(*pb)); pa++, pb++) ;
  return (pa-a == len || *pa == *pb);
}
inline void mi_lowercase(char *buf) { for (char *p = buf; *p != 0; p++) *p = tolower(*p); }
